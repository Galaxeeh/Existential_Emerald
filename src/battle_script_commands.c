...
{
    FLAG_WOODSGRUNT, FLAG_BADGE01_GET, FLAG_BEAT_WALLY_V, FLAG_BADGE02_GET, FLAG_BEAT_CYRUS1, FLAG_BADGE03_GET, FLAG_BADGE04_GET,
    FLAG_BADGE05_GET, FLAG_BADGE06_GET, FLAG_BADGE07_GET, FLAG_BADGE08_GET,
};

const u16 sLevelCaps[NUM_SOFT_CAPS] = { 10, 16, 22, 27, 35, 38, 42, 50, 60, 70, 80 };
...
static bool32 AccuracyCalcHelper(u16 move)
{
    if ((gStatuses3[gBattlerTarget] & STATUS3_ALWAYS_HITS && gDisableStructs[gBattlerTarget].battlerWithSureHit == gBattlerAttacker)
     ...
     || gStatuses4[gBattlerTarget] & STATUS4_GLAIVE_RUSH || (move == MOVE_THUNDER_WAVE && IS_BATTLER_OF_TYPE(gBattlerAttacker, TYPE_ELECTRIC)))
    {
        JumpIfMoveFailed(7, move);
        return TRUE;
    }
...
s32 CalcCritChanceStageArgs(u32 battlerAtk, u32 battlerDef, u32 move, bool32 recordAbility, u32 abilityAtk, u32 abilityDef, u32 holdEffectAtk)
{
    ...
    if (gSideStatuses[battlerDef] & SIDE_STATUS_LUCKY_CHANT
       || abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR || abilityDef == ABILITY_MAGMA_ARMOR || abilityDef == ABILITY_LEAF_GUARD)
    {
        critChance = -1;
    }
    ...
    else
    {
        ...
        if (critChance >= 3 && recordAbility && (abilityDef == ABILITY_BATTLE_ARMOR || abilityDef == ABILITY_SHELL_ARMOR || abilityDef == ABILITY_MAGMA_ARMOR || abilityDef == ABILITY_LEAF_GUARD))
